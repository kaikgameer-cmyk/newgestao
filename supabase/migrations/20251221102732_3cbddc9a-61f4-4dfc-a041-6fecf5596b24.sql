-- Adjust finalize_competition to use conditional winner messages for team vs individual
...
BEGIN
  ...
  ELSE
    ...
    IF v_meta_reached AND v_winner_user_id IS NOT NULL THEN
      ...
      -- Resolve winner display name for host message
      SELECT COALESCE(TRIM(CONCAT(p.first_name, ' ', p.last_name)), p.name, 'Usuário')
      INTO v_winner_user_name
  ...
  IF v_meta_reached THEN
    ...
    -- Host message depends on mode
    IF v_winner_type = 'team' THEN
      ...
      VALUES (
        ...
        jsonb_build_object(
          ...
          'message', 'A competição acabou. O time vencedor foi ' || COALESCE(v_winner_team_name, 'seu time vencedor') || '. Faça o pagamento dos ganhadores.'
        )
      )
      ON CONFLICT (user_id, competition_id, type) DO NOTHING;
    ELSIF v_winner_type = 'individual' THEN
      INSERT INTO public.notifications (user_id, type, competition_id, payload)
      VALUES (
        v_competition.created_by,
        'competition_host_payout',
        ...
        jsonb_build_object(
          ...
          'winner_user_name', v_winner_user_name,
          'winners_count', 1,
          'winners', COALESCE(v_winners, '[]'::jsonb),
          'message', 'A competição acabou. O vencedor foi ' || COALESCE(v_winner_user_name, 'o ganhador') || '. Faça o pagamento do prêmio.'
        )
      )
      ON CONFLICT (user_id, competition_id, type) DO NOTHING;
    END IF;
  ...
  IF v_meta_reached THEN
    -- Winners (conditional message based on mode)
    IF v_winner_type = 'team' THEN
      INSERT INTO public.notifications (user_id, type, competition_id, payload)
      SELECT 
        cp.user_id,
        ...
        jsonb_build_object(
          ...
          'winner_team_name', v_winner_team_name,
          'winners_count', v_winner_count,
          'message', 'Seu time venceu a competição ' || v_competition.name || ' e atingiu a meta. Seu valor individual do prêmio é R$ ' || cp.payout_value::text || ' (prêmio dividido entre os membros do time vencedor). O host vai entrar em contato para combinar o pagamento.'
        )
      FROM public.competition_payouts cp
    ...
    ELSIF v_winner_type = 'individual' THEN
      ...
      SELECT 
        ...
        jsonb_build_object(
          ...
          'winner_type', 'individual',
          'winners_count', 1,
          'message', 'Você venceu a competição ' || v_competition.name || ' e atingiu a meta. Seu prêmio é R$ ' || cp.payout_value::text || '. O host vai entrar em contato para combinar o pagamento.'
        )
      FROM public.competition_payouts cp
      WHERE cp.competition_id = p_competition_id
        AND cp.status = 'winner'
    ...
    -- Losers (meta atingida, mas não ganharam) - mensagem permanece a mesma
    INSERT INTO public.notifications (user_id, type, competition_id, payload)
    SELECT 
      cp.user_id,
